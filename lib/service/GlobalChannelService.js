...
class GlobalChannelService {
  ...
  async pushMessageForUid(route, msg, uids, serverType, frontServerId) {
    ...
    return await GlobalChannelServiceUtility.RpcInvoke(
      this.app,
      frontServerId,
      route,
      msg,
  ...
  async pushMessageByUidArr(uidArr, route, msg, frontServerId = null) {
    ...
    _.forEach(records, (sid, uid) => {
      if (uid.size > 0) {
        sendMessageArr.push(
          GlobalChannelServiceUtility.RpcInvoke(
            this.app,
            sid,
            route,
            msg,
  ...
  async pushMessage(serverType, route, msg, channelName) {
    ...
    const uidObject = await this.getMembersByChannelName(
      serverType,
      channelName
    );
    let failIds = [];
    ...
    _.forEach(uidObject, (sid, uids) => {
      if (uids && uids.length > 0) {
        coRpcArr.push(
          GlobalChannelServiceUtility.RpcInvoke(this.app, sid, route, msg, uids)
        );
      }
    });
    if (coRpcArr.length > 0) {
      return await Promise.all(coRpcArr);
    }
    return [];
  }
